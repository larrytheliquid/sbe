...
xs   &: \gmenv{\Xi} && \byass\\
x    &: \xtypm{A} && \byass\\
xs'  &: \gmenv{(\Xi, \Delta)} && \ih{\monos~\Delta~xs}\\
x'   &: \xdtypm{A} && \by{\mono~\Delta~x}\\
xs'' &: \cmenv{(\Gamma, A)}{(\Xi, \Delta)} && \by{xs', x'}
\end{align*}
\end{lcase}
...
\todo[inline]{description of environment model monotonicity}
...
\Gamma& && \byass\\
A& && \byass\\
xs   &: \cmenv{\Gamma}{\Gamma} && \ih{\midenv~\Gamma}\\
xs'  &: \cmenv{\Gamma}{(\Gamma, A)} && \by{\monos~(\emp, A)~xs}\\
x    &: \gatypm{A} && \by{\reflect~(\var~\here)}\\
xs'' &: \cmenv{(\Gamma, A)}{(\Gamma, A)} && \by{xs', x}
\end{align*}
\end{lcase}
...
\todo[inline]{description of identity model environment}
