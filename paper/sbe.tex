...
\def\midenvT{\all{\Gamma} \gmenv{\Gamma}}
...
translation from a syntactic value in some initial context
($\Gamma$), to a model value
in some terminal context ($\Delta$). In addition to the value parameter,
canonical evaluation also requires an environment of model values
($\menv$). Each model value must be in the terminal context ($\Delta$), and
...
the return type is a model-level function. This means that
we get a model value from the domain of the model function as an
additional parameter, which we
can subsequently extend the model environment with. Lucky for us, because
...
Canonical evaluation of neutrals ($\cevaln$) is the restriction of
canonical evaluation of values ($\cevalv$) to the neutral fragment of
...
primitive recursion in the model using a lemma ($\mrec$).
}
\begin{align*}
c_z  &: \typv{C} && \byass\\
