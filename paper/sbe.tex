...
result remains a canonical term. As such, hereditary substitution can
be used to give an alternative semantics for expressions (e.g. instead
of a small-step semantics), by normalizing the expression to a
canonical term and invoking hereditary substitution in the function
application case.
...
termination argument of hereditary substitution that easily scales to
...
Canonical Evaluation (SbE), and much of the existing NbE machinery can be reused, or
only requires minor modifications. In some sense, we merely point out
and clarify a beautiful coincidence: NbE for canonical terms is essentially a
...
definition (or implicit termination proof) of NbE. Then, we present the
...
works on values instead of expressions. In SbE, canonical evaluation
...
\todo[inline]{talk about the beta redexes in syntactic hsub}
...
\todo[inline]{ceval is not just composition bc of extra env argument}
...
expressions turned syntactic expressions into semantic values, reducing
...
initial context ($\typv{A}$) and a semantic environment ($\menv$), and produces a
...
parameter is a semantic value that has been weakened by $\Xi$. See the
...
the reason why canonical evaluation requires a semantic environment parameter in
...
semantic environment, by appealing to the monotonicity lemma for
environments ($\monos$).
}
\begin{align*}
b   &: \gatypv{B} && \byass\\
...
to model environment monotonicity (\reflem{monos}) to weaken the
...
The value monotonicity lemma is basically a version of weakening (of
syntactic values) lifted to semantic values.
