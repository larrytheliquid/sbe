...
The model of environments is a mapping from all variables in some initial context
...
For context extension (extending $\Gamma$ by type $A$), the
environment model must contain an element
of the value model in the terminal context ($\Delta$) of the
environment. Additionally, the model of the environment for the
context being extended ($\Gamma$) is also required.
